...
By 2021, 50 percent of all enterprise workloads were already hosted in \emph{public clouds} \autocite{Security_Compass2021-sh}. As of 2025 the global cloud computing market size is valued at USD 912.77 billion and is expected to grow from 2026 to 2035 at a compound annual growth rate of 20.61\% \autocite{PrecedenceResearchCloudMarketSize2025-zd}.
...
In this thesis we aim to develop a practical and secure solution for the development of semantic data knowledge bases, which remain queryable after being encrypted. Our main contributions are the following:


\begin{description}
    \item \textbf{SPARQL-EVAL \& SPARQL-EVAL-II - Distributed two-party \gls{SPARQL} query evalua-}\linebreak \textbf{tion protocol:} We developed two \gls{SPARQL} query evaluation protocols which support a subset of queries: queries target solely the default graph; the allowed modifier are DISTINCT, ORDERBY and LIMIT; graph patterns must not contain property paths, nested subqueries, graph selection, nor triple patterns composed solely by variables. In SPARQL-EVAL-II we use query rewriting techniques to retain reasoning capabilities. 
    ...
    \item \textbf{\gls{SEnTriQ} - A distributed three-party \gls{SPARQL} query evaluation protocol} \linebreak \textbf{ for SSE-TP encrypted triplestores:}   
    \gls{SEnTriQ} is an adaptation of the SPARQL-EVAL-II protocol that works with SSE-TP encrypted triplestores. The protocol is implemented in two versions:
    \begin{itemize} 
    
        \item \emph{\gls{SEnTriQ} V1}: Data encryption retains elements from \emph{CryptDB} \autocite{Popa2011-zb, Popa2012-bw, Popa2011-ci}. Subjects, predicates and objects are encrypted in two layers. The outer layer is probabilistically encrypted, whereas the second is deterministically encrypted. When equality checks are required, we peel the outer layer of the encryption onions and compare the values. This implementation achieves \gls{IND-CKA1} security guaranties.
    
        \item \emph{\gls{SEnTriQ} V2}: We use multiplicative blinding techniques with the aim of reducing leakage during query evaluation. The value of a subject, predicate or object is mapped to an \emph{equality tag}, an integer value encrypted with the \gls{DGK} cryptosystem \autocite{Damgard2007-km, Damgard2009-my, Franz2011-sm, Makkes2010-qb}. Trapdoors index the equality tags and equality tags reference probabilistically encrypted subjects, predicates and objects. During query evaluation, we mask the values of the equality tags, via multiplicative blinding. Equality checks are done by partially decrypting the blinded encryption tags, stripping only the randomized layer of the ciphertexts. These values remain deterministic but different for every query evaluation. This implementation achieves \gls{IND-CKA2} security guaranties.
    \end{itemize}
\end{description}

The implementation of the contributions presented in this thesis is publicly available at \url{https://github.com/DCebola/SEnTriS}. 
...
\begin{description}
    ...
    \item [Chapter 5] evaluates the solutions in terms of complexity and security. It also presents the experimental evaluation procedures, describes and discusses the obtained results.
