...
\begin{description}
    ...
    The main distinction between them is in the equality function used during set operations. In the first version, when equality checks are required, we peel the outer layer of the encryption onions and compare the values. In the second version, we mask the values of the equality tags, via re-encryption. When doing equality checks, we decrypt the \emph{blinded equality tags} and compare the values. These remain deterministic, but different for every query evaluation.
