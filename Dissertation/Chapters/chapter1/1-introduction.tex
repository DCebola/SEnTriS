...
\begin{description}
    ...
    \begin{itemize} 
        ...
        \item \emph{\gls{SEnTriS} V2}: We use multiplicative blinding techniques with the aim of reducing leakage during query evaluation. The value of a subject, predicate or object is mapped to an \emph{equality tag}, an integer value encrypted with the \gls{DGK} cryptosystem \autocite{Damgard2007-km, Damgard2009-my, Franz2011-sm, Makkes2010-qb}. Trapdoors index the equality tags and equality tags reference probabilistically encrypted subjects, predicates and objects. During query evaluation, we mask the values of the equality tags, via multiplicative blinding. Equality checks are done by partially decrypting the blinded encryption tags, stripping only the randomized layer of the ciphertexts. These values remain deterministic but different for every query evaluation. This implementation achieves \gls{IND-CKA2} security guaranties.
    \end{itemize}
\end{description}
