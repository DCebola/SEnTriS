...
In the present times, data is one of the backbones of the modern economy. However, data is as useful it can be interpreted. Hence the importance of knowledge representation, and consequently of ontologies.
...
Consequently, in this thesis, we focused on answering the following two questions: 1) \emph{How to support secure and expressive read and write \gls{SPARQL} queries over encrypted data?}; 2)\emph{How can we add reasoning capabilities to the solution?}
...
\gls{SEnTriQ} shows that, by using a combination of \gls{SSE} encrytion techniques, it is possible to develop a solution capable of evaluating a subset of read and write \gls{SPARQL} queries, againts encrypted data. It also demonstrates that, by using query rewritting techniques, such solution can keep its reasoning capabilities.

\section{Future Work}
\label{sec:future-work}

Although \gls{SEnTriQ} proved that it is possible to evaluate \gls{SPARQL} queries against encrypted datasets, it is still very primordial work, and possesses a lot of limitation. 

In no particular order, we present future lines of research ... :
...
\paragraph*{Research other query rewritting tehcniques} The algorithm developed requires bounding parameters, otherwise it continuously expands a query. Additionally, it does expansion with very basic approach. Research for the usage of more sophisticated rule engines, or other expansion techniques should be conducted.

\paragraph*{Support for named graphs and multiple ontologies} \gls{SEnTriQ} can only evaluate queries agains a default triplestore, using a single ontology. Furthermore, it does not support partial updates of the ontologies, nor support for combined usage of multiple ontologies during query evaluation.
...
\paragraph*{Extension to the \gls{M/M} setting} The support for multiple users in the implemented prototype was done via \textit{secret sharing}. It is relevant to research how to adapt \gls{SEnTriQ} to securily support multiple users, distribute secrets, and revoke deleted users.

\paragraph*{Performance Research} The developed algorithms were designed with the intent to minimise the complexity overhead of the cryptographic primitives. Nevertheless, performance was not the major focus of this thesis. It would be valuable to investigate the usage of other cryptographic primitives and different query planning schemes (e.g. that use heuristics) or query evaluation schemes (e.g. using concurrent \emph{query workers}, introducing parallelization).
...
\paragraph*{Develop a more realistic prototype} The implemented prototype works in a local development environment. Evaluation of a pratical prototype, whose components run in a cloud environment, would be valuable to understand running costs and trully assess its practically.

\paragraph*{Strengthening the security model}
While the current system provides different levels of confidentiality, future work could study stronger adversarial models, refined leakage analysis, and additional countermeasures against access-pattern and frequency-based attacks.