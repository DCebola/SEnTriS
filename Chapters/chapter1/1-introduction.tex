...
As of 2021, cloud adoption as risen more than what was previously forecasted, with 50 percent of all enterprise workloads hosted in \emph{public clouds} and predictions of 7 percentage point gain in 2022 \autocite{Security_Compass2021-sh}.  Research on secure cloud computing \autocite{Carroll2011-me} explains such adoption but also discusses security concerns. Adoption can be explained with the cost efficiency (reduced implementation and maintenance, power, cooling, floor space, storage and operational costs), high scalability, flexibility and availability that cloud provides. Additionally, the pay-per-use model allows for an easy entrance to the ecosystem.

%Cloud -> including high availability, elasticity, and ubiquity, pay-as-you-go model, the elimination of concerns relative to maintenance, high availability, and cheap geo-replication at the loss of privacy and control. trusting its owners, a problem many end-users are aware. Recent news have proven data stored on Cloud servers is susceptible to privacy leaks and data breaches... Especially concerning with sensitive data like health records

%Considering honest-but-curious adversary, which observes all data and computations on the Cloud server without interfering; such model implies that sensitive data can never be decrypted on the server

%Classical approach -> Encryption at Rest (Encripted in the cloud, download, decrypted at the client for updates and searches) inneficient and require high-network and client-side computations
