...
for i in $(docker ps -a --filter="ancestor=$1/sentri-vault-api" --format "{{.ID}}")
do
    docker rm $(docker stop $i) &> /dev/null
    wait
done
for i in $(docker image ls "$1/sentri-vault-api" --format "{{.ID}}")
do
...
cd ../Deployment/Vault
docker buildx build -t $1/sentri-vault-api .
wait
